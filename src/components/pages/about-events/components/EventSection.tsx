      ...
      <div className="flex flex-row w-full max-w-6xl md:hidden gap-2 px-2 items-start">
        <div className="flex flex-col w-1/2 gap-20">
          {events.filter((_, i) => i % 2 === 0).map((event, i) => (
            <EventCard
              key={`mob-left-${i}`}
              imageSrc={resolveImageSrc(event.image)}
              title={event.title}
              date={event.date}
              className="w-full"
            />
        ...
        {events.map((event, index) => {
          ...
          return (
            <EventCard
              ...
              title={event.title}
              date={event.date}
              className={`md:scale-50 md:-my-65 -translate-x-1 ${
                isShiftedRow ? "md:translate-x-44" : "md:-translate-x-32"
              }`}
            />
          );
        })}
